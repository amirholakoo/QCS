...
import { authAPI, paperAPI, pulpAPI, materialAPI, logsAPI, reportAPI, productionMachineAPI, paperTypeAPI, speedAPI } from '../utils/api';
import type { User, Paper, Pulp, Material, LogEntry, ProductionMachine, PaperTypeItem, Speed } from '../types';
...
export const usePermissions = () => {
  return useAPI<{ permissions: Record<string, { view: boolean; add: boolean; change: boolean; delete: boolean }> }>(
...
// Paper Type hooks
export const usePaperTypes = (params?: Record<string, string>) => {
  return useAPI<{ results: PaperTypeItem[]; count: number }>(
    () => paperTypeAPI.list(params),
    [JSON.stringify(params)]
  );
};

export const usePaperType = (id: string) => {
  return useAPI<PaperTypeItem>(() => paperTypeAPI.get(id), [id]);
};
...
// Paper Type mutation hooks
export const useCreatePaperType = () => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const createPaperType = async (data: any) => {
    try {
      setLoading(true);
      setError(null);
      const result = await paperTypeAPI.create(data);
      return result;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'خطا در ایجاد نوع کاغذ';
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
  ...
  return { createPaperType, loading, error };
};
...
export const useUpdatePaperType = () => {
  ...
  const updatePaperType = async (id: string, data: any) => {
    try {
      setLoading(true);
      setError(null);
      const result = await paperTypeAPI.update(id, data);
      return result;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'خطا در ویرایش نوع کاغذ';
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
  ...
  return { updatePaperType, loading, error };
};

export const useDeletePaperType = () => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const deletePaperType = async (id: string) => {
    try {
      setLoading(true);
      setError(null);
      await paperTypeAPI.delete(id);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'خطا در حذف نوع کاغذ';
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
  ...
  return { deletePaperType, loading, error };
};
...
// Speed hooks
export const useSpeeds = (params?: Record<string, string>) => {
  return useAPI<{ results: Speed[]; count: number }>(
    () => speedAPI.list(params),
    [JSON.stringify(params)]
  );
};

export const useSpeed = (id: string) => {
  return useAPI<Speed>(() => speedAPI.get(id), [id]);
};

export const useCreateSpeed = () => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const createSpeed = async (data: any) => {
    try {
      setLoading(true);
      setError(null);
      const result = await speedAPI.create(data);
      return result;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'خطا در ایجاد سرعت';
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
  ...
  return { createSpeed, loading, error };
};

export const useUpdateSpeed = () => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const updateSpeed = async (id: string, data: any) => {
    try {
      setLoading(true);
      setError(null);
      const result = await speedAPI.update(id, data);
      return result;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'خطا در ویرایش سرعت';
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
  ...
  return { updateSpeed, loading, error };
};

export const useDeleteSpeed = () => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const deleteSpeed = async (id: string) => {
    try {
      setLoading(true);
      setError(null);
      await speedAPI.delete(id);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'خطا در حذف سرعت';
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
  ...
  return { deleteSpeed, loading, error };
};
