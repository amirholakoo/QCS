...
export const authAPI = {
  ...
  getSystemVersion: () =>
    apiRequest('/auth/system-version/'),

  getUpdatingStatus: () =>
    apiRequest('/auth/updating-status/'),
  getPermissions: () =>
...
export const paperAPI = {
  ...
  restore: (id: string) =>
    apiRequest(`/paper/records/${id}/restore/`, {
      method: 'POST',
    }),
  ...
  // Get last roll for a production line (or overall if omitted)
  lastRoll: (params?: Record<string, string>) => {
    const queryString = params ? '?' + new URLSearchParams(params).toString() : '';
    return apiRequest(`/paper/records/last_roll/${queryString}`);
  },

  getTodayDate: () =>
    apiRequest('/paper/records/today/'),

  getPmNoteDefaults: (): Promise<Record<string, Record<string, string>>> =>
    apiRequest('/paper/records/pm_note_defaults/'),

  exportCsv: (params?: Record<string, string>) => {
    const queryString = params ? '?' + new URLSearchParams(params).toString() : '';
    return downloadFile(`/paper/records/export_csv/${queryString}`);
  },
  validateShamsiDate: (date: string) => {
    const queryString = '?date=' + encodeURIComponent(date);
    return apiRequest(`/paper/records/validate_shamsi_date/${queryString}`);
  },
};
...
const downloadFile = async (endpoint: string, filename?: string): Promise<void> => {
  const url = `${API_BASE_URL}${endpoint}`;
  // Try to trigger a browser-managed download immediately using a hidden iframe.
  // This lets the browser stream the response and start the download without
  // buffering the entire file in JS. If that fails (CORS, popup blocked),
  // fallback to fetching the blob and saving it.
  try {
    if (typeof window !== 'undefined' && document) {
      // Use top-level navigation via an anchor click so cookies/credentials are sent
    ...
    console.warn('Direct link download failed, falling back to blob fetch', err);
  }

  // Fallback: fetch the file and trigger download via blob (keeps previous behaviour)
  try {
    ...
    link.download = finalFilename || 'download.csv';
    document.body.appendChild(link);
    link.click();
...
export const pulpAPI = {
  ...
  restore: (id: string) =>
    apiRequest(`/pulp/records/${id}/restore/`, {
      method: 'POST',
    }),
  
  exportCsv: (params?: Record<string, string>) => {
    const queryString = params ? '?' + new URLSearchParams(params).toString() : '';
    return downloadFile(`/pulp/records/export_csv/${queryString}`);
  },
  validateShamsiDate: (date: string) => {
    const queryString = '?date=' + encodeURIComponent(date);
    return apiRequest(`/paper/records/validate_shamsi_date/${queryString}`);
...
export const paperTypeAPI = {
  ...
  restore: (id: string) =>
    apiRequest(`/paper-type/records/${id}/restore/`, {
      method: 'POST',
    }),
};
...
export const materialAPI = {
  ...
  restore: (id: string) =>
    apiRequest(`/material/records/${id}/restore/`, {
      method: 'POST',
    }),
};
...
export const reportAPI = {
  ...
  exportCompleteReportCsv: (params?: Record<string, string>) => {
    const queryString = params ? '?' + new URLSearchParams(params).toString() : '';
    return downloadFile(`/report/complete-report-export-csv/${queryString}`);
  },
  ...
  // Backward-compatible: keep the old name but point to CSV streaming endpoint
  exportCompleteReportXlsx: (params?: Record<string, string>) => {
    return reportAPI.exportCompleteReportCsv(params);
  },

  getPLCKeys: () =>
    apiRequest('/report/plc-keys/'),

  reorderPLCKeys: (orderedIds: number[]) =>
    apiRequest('/report/plc-keys/reorder/', {
      method: 'POST',
...
export const qcAPI = {
  ...
  restoreRecord: (id: string) =>
    apiRequest(`/qc/records/${id}/restore/`, {
      method: 'POST',
    }),
  ...
  getWarehouseNames: () =>
    apiRequest('/qc/records/warehouse_names/'),

  getWarehouseWidths: (anbarLocation: string) =>
    apiRequest(`/qc/records/warehouse_widths/?anbar_location=${encodeURIComponent(anbarLocation)}`),

  getWarehouseReels: (
    anbarLocation: string,
    width: number,
    excludeQcId: string | undefined = undefined,
    searchQuery: string = '',
    onlyUnused: boolean = false
  ) => {
    let url = `/qc/records/warehouse_reels/?anbar_location=${encodeURIComponent(anbarLocation)}&width=${encodeURIComponent(String(width))}`;
    if (excludeQcId) {
      url += `&exclude_qc_id=${excludeQcId}`;
    }
    if (searchQuery) {
  ...
  acquireEditLock: (id: string) =>
    apiRequest(`/qc/records/${id}/acquire_edit_lock/`, {
      method: 'POST',
    }),

  releaseEditLock: (id: string) =>
    apiRequest(`/qc/records/${id}/release_edit_lock/`, {
      method: 'POST',
    }),
  ...
  restoreCustomer: (id: string) =>
    apiRequest(`/qc/customers/${id}/restore/`, {
      method: 'POST',
    }),
  ...
  restoreLoading: (id: string) =>
    apiRequest(`/qc/loading/${id}/restore/`, {
      method: 'POST',
    }),
};
...
export const speedAPI = {
  list: (params?: Record<string, string>) => {
    const queryString = params ? '?' + new URLSearchParams(params).toString() : '';
    return apiRequest(`/speed/records/${queryString}`);
  },
  
  create: (data: any) =>
    apiRequest('/speed/records/', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  
  update: (id: string, data: any) =>
    apiRequest(`/speed/records/${id}/`, {
      method: 'PUT',
      body: JSON.stringify(data),
    }),
  ...
  restore: (id: string) =>
    apiRequest(`/speed/records/${id}/restore/`, {
      method: 'POST',
    }),
};