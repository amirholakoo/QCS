...
import { ArrowRight, Trash2, ArrowLeft, X } from 'lucide-react';
...
export const SpeedForm: React.FC<SpeedFormProps> = ({ speed, onSave, onCancel, onDelete, readOnly = false }) => {
  ...
  const speedPerms = permissionsData?.permissions?.speed || { view: false, add: false, change: false, delete: false };
  ...
  const handleDeleteConfirm = async () => {
    if (!speed) return;
    const deletedId = String(speed.id);
    try {
      await deleteSpeed(deletedId);
  ...
  const handleRestoreSpeed = async () => {
    ...
    setRestoring(true);
    try {
      await speedAPI.restore(justDeleted);
        ...
        {!readOnly && (
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              ...
              {((isEditing && speedPerms.change) || (!isEditing && speedPerms.add)) && (
