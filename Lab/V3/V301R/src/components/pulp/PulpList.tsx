...
import { useInfiniteScroll, useDeletePulp, usePermissions } from '../../hooks/useAPI';
...
export const PulpList: React.FC<PulpListProps> = ({ onEdit, onView, onCreate, onRefetch }) => {
  ...
  useEffect(() => {
    const fetchLocationNames = async () => {
      try {
        ...
        showToast('error','خطا در دریافت محل های نمونه گیری | پس از اطمینان به اتصال اینترنت دوباره وارد این بخش شوید');
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      await pulpAPI.exportCsv(params);
  ...
  // Reuse the same formatter used in PaperList: format YYYY-MM-DD as user types
  ...
  const validateDateServer = async (date: string, label: string) => {
    if (!date) return;
    try {
      // use pulpAPI validation mapping (falls back to paper endpoint)
      await pulpAPI.validateShamsiDate(date);
