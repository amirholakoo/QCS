...
import { Search, Loader, Eye, Edit, Download, RefreshCw, ChevronDown, MessageSquare } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import type { Paper, Pulp, PLCKey, RollPLCData, PM_Setting, Material, Speed } from '../../types';
import { useInfiniteScroll, useMaterials, useAPI } from '../../hooks/useAPI';
import { paperAPI, pulpAPI, reportAPI, speedAPI } from '../../utils/api';
...
import { SPEED_FIELDS } from '../speed/speedFields';
...
interface CompleteReportProps {
  ...
  onViewSpeed?: (speed: Speed) => void;
  onEditSpeed?: (speed: Speed) => void;
}

type RowType = 'paper' | 'pulp' | 'speed';
...
export const CompleteReport: React.FC<CompleteReportProps> = ({
  ...
  const [activeHeaderType, setActiveHeaderType] = useState<'paper' | 'pulp' | 'speed'>('paper');
  ...
  // Formatter to help users type YYYY-MM-DD (Jalali)
  ...
  const validateDateServer = async (date: string, label: string) => {
    if (!date) return;
    try {
      await reportAPI.validateShamsiDate(date);
  ...
  const {
    data: speeds,
    loading: speedsLoading,
    error: speedsError,
    hasMore: speedsHasMore,
    refetch: refetchSpeeds,
  } = useInfiniteScroll<Speed>(speedAPI.list, {}, 'all');
  ...
  const [plcKeysOrdered, setPlcKeysOrdered] = useState<PLCKey[]>([]);
  const draggingPlcKeyIdRef = useRef<number | null>(null);

  useEffect(() => {
    const keys = plcKeysResponse?.plc_keys || [];
    setPlcKeysOrdered(keys);
  }, [plcKeysResponse]);

  const plcKeys = plcKeysOrdered;
  ...
  activePlcKeys.sort((a, b) => {
    return a.order_index - b.order_index;
  });
  // let itemToMove = activePlcKeys.find(item => item.key === "me1");
  // // If found, remove it from its original position
  // if (itemToMove) {
  //     activePlcKeys = activePlcKeys.filter(item => item.key !== "me1");

  //     // Set the new order_index for the item
  //     itemToMove.order_index = 3;

  //     // Insert the item at the desired position (index 3)
  //     // Note: JavaScript arrays are 0-indexed, so index 3 means it will be the 4th element.
  //     // If you want it to be the 3rd element, you would use splice(2, 0, itemToMove)
  //     activePlcKeys.splice(3, 0, itemToMove);

  //     // Re-index the remaining items to ensure sequential order
  //     activePlcKeys.forEach((item, index) => {
  //         // We add 1 because order_index is typically 1-based, and array index is 0-based.
  //         // Adjust this if your order_index convention is different.
  //         item.order_index = index + 1;
  //     });
  // }
  const movePlcKey = useCallback(
    async (dragId: number, overId: number) => {
      if (dragId === overId) return;

      const fromIndex = plcKeysOrdered.findIndex((k) => k.id === dragId);
      const toIndex = plcKeysOrdered.findIndex((k) => k.id === overId);
      if (fromIndex === -1 || toIndex === -1) return;

      const next = [...plcKeysOrdered];
      const [moved] = next.splice(fromIndex, 1);
      next.splice(toIndex, 0, moved);
      const normalized = next.map((k, idx) => ({ ...k, order_index: idx + 1 }));

      setPlcKeysOrdered(normalized);

      try {
        const orderedIds = normalized.map((k) => k.id);
        await reportAPI.reorderPLCKeys(orderedIds);
        await refetchPlcKeys();
      } catch (error) {
        showToast('error','خطا در مرتب سازی کلید های plc | اگر از اتصال اینترنت خود مطمئن هستید این مشکل را به واحد فنی گزارش کنید');
        console.error('Failed to reorder PLC keys:', error);
        await refetchPlcKeys();
      }
    },
    [plcKeysOrdered, refetchPlcKeys]
  );
  ...
  useEffect(() => {
    const fetchLocationNames = async () => {
      try {
        ...
        showToast('error','خطا در دریافت محل های نمونه گیری | اگر از اتصال اینترنت خود مطمئن هستید این مشکل را به واحد فنی گزارش کنید.');
  ...
  useEffect(() => {
    const loadPreference = async () => {
      ...
      try {
        ...
        showToast('error','خطا در بارگذاری کلید های دلخواه plc | همه کلید های به عنوان پیش فرض نمایش داده میشوند');
  ...
  useEffect(() => {
    const syncOnMount = async () => {
      try {
        ...
        showToast('error','نا موفق در همگام سازی داده های PLC | این مشکل را به واحد فنی گزارش کنید');
  ...
  const speedsList = Array.isArray(speeds) ? speeds : [];
  ...
  const matchedData = useMemo(() => {
    ...
    papers.forEach(paper => {
      ...
      const matchingSpeeds = speedsList.filter(speed => {
        const rollPaper = (paper.roll_number ?? '').toString().trim();
        const rollSpeed = (speed.Roll_Number ?? '').toString().trim();
        return rollPaper !== '' && rollSpeed !== '' && rollPaper === rollSpeed;
      });

      matchingSpeeds.forEach(speed => {
        rows.push({
          type: 'speed',
          speed,
          paper,
  ...
  const loading = papersLoading || pulpsLoading || speedsLoading || plcKeysLoading || rollPlcLoading;
  const error = papersError || pulpsError || speedsError || plcKeysError || rollPlcError;
  ...
  if (error) {
    return (
      <div className="text-center text-red-600 p-8">
        ...
        <button onClick={() => { refetchPapers(); refetchPulps(); refetchSpeeds(); }} className="btn-primary mt-4">
          {t('common.retry')}
        </button>
      </div>
    );
  ...
  const getSpeedHeader = () => {
    const paperColumns = 22 + groupedPlcKeys.length;
    const speedColumns = 4 + SPEED_FIELDS.length;
    const paddingColumns = Math.max(0, paperColumns - speedColumns);
    return (
      <thead className="sticky top-0 z-10 bg-gray-50">
        <tr>
          <th className="bg-gray-50 whitespace-nowrap">{t('paper.operations')}</th>
          <th className="bg-gray-50 whitespace-nowrap">{t('speed.rollNumber')}</th>
          <th className="bg-gray-50 whitespace-nowrap">{t('speed.date')} - {t('speed.time')}</th>
          <th className="bg-gray-50 whitespace-nowrap">{t('speed.lastUpdated')}</th>
          {SPEED_FIELDS.map(({ labelKey }) => (
            <th key={labelKey} className="bg-gray-50 whitespace-nowrap min-w-[80px] text-center">{labelKey}</th>
          ))}
          {Array.from({ length: paddingColumns }).map((_, index) => (
            <th key={`pad-${index}`} className="bg-gray-50 whitespace-nowrap"></th>
          ))}
  ...
  const renderSpeedRow = (speed: Speed) => {
    const paperColumns = 22 + groupedPlcKeys.length;
    const speedColumns = 4 + SPEED_FIELDS.length;
    const paddingColumns = Math.max(0, paperColumns - speedColumns);
    return (
      <tr
        key={`speed-${speed.id}`}
        className="table-row-hover bg-amber-50"
        onClick={() => handleRowClick({ type: 'speed', speed, id: `speed-${speed.id}` })}
        style={{ cursor: 'pointer' }}
      >
        <td>
          <div className="items-center gap-2">
            {onViewSpeed && (
              <button
                onClick={(e) => { e.stopPropagation(); onViewSpeed(speed); }}
                className="text-blue-600 hover:text-blue-700 p-1 rounded hover:bg-blue-50"
                title={t('common.view')}
              >
                <Eye className="w-4 h-4" />
            ...
            {onEditSpeed && (
              <button
                onClick={(e) => { e.stopPropagation(); onEditSpeed(speed); }}
                className="text-primary-600 hover:text-primary-700 p-1 rounded hover:bg-primary-50"
                title={t('common.edit')}
              >
                <Edit className="w-4 h-4" />
                    ...
                    {plcKeys.map((key) => {
                      ...
                      return (
                        <button
                          ...
                          draggable
                          onDragStart={(e) => {
                            draggingPlcKeyIdRef.current = key.id;
                            e.dataTransfer.effectAllowed = 'move';
                          }}
                          onDragEnd={() => {
                            draggingPlcKeyIdRef.current = null;
                          }}
                          onDragOver={(e) => {
                          ...
                          onDrop={(e) => {
                            e.preventDefault();
                            const dragId = draggingPlcKeyIdRef.current;
                            draggingPlcKeyIdRef.current = null;
                            if (dragId == null) return;
                            void movePlcKey(dragId, key.id);
                          }}
          ...
          {matchedData.length > 0 ? (
            <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
              <table className="table">
                {activeHeaderType === 'paper' ? getPaperHeader() : activeHeaderType === 'pulp' ? getPulpHeader() : getSpeedHeader()}
                <tbody>
                  {matchedData.map((row, index) => {
                    ...
                    } else if (row.type === 'speed' && row.speed) {
                      return renderSpeedRow(row.speed);
                    }
                    return null;
                  })}
                </tbody>
