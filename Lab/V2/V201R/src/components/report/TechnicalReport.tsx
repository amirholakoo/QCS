...
export const TechnicalReport: React.FC = () => {
    ...
    try {
      ...
      if (chartResult.success && chartResult.series) {
        ...
        if (timeFilter === 'monthly' && chartResult.roll_numbers) {
          ...
          // Filter each chart's data to only include points with actual data
          setChartData(filterSeriesData(paperData, paperRolls));
          setTopHeadboxData(filterSeriesData(topHeadboxData, topHeadboxRolls));
          setBottomHeadboxData(filterSeriesData(bottomHeadboxData, bottomHeadboxRolls));
          setTensileData(filterSeriesData(tensileData, tensileRolls));
          setConsData(filterSeriesData(consData, consRolls));
        }
      }
    } catch (err) {
  ...
  const chartOptions = useMemo(() => ({
      ...
      custom: function({ seriesIndex, dataPointIndex, w }: any) {
        ...
        if (data.y === null || data.y === undefined) {
          return `
            <div class="custom-tooltip" style="text-align: right;background: white; padding: 12px; border: 1px solid #ccc; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
              ...
              <div style="margin-bottom: 8px; color: #999; font-style: italic;">داده موجود نیست</div>
              <div style="border-top: 1px solid #E5E7EB; padding-top: 8px; color: #374151; font-weight: 600; text-align: center;">شماره رول: ${data.rollNumber}</div>
            </div>
          `;
        }
        ...
        return `
          <div class="custom-tooltip" style="text-align: right;background: white; padding: 12px; border: 1px solid #ccc; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            ...
            <div style="margin-bottom: 8px; color: #6B7280;">زمان پایان نمونه‌گیری: <span style="font-weight: 600; color: #111827;">${data.samplingEndTime || 'نامشخص'}</span></div>
            <div style="border-top: 1px solid #E5E7EB; padding-top: 8px; color: #374151; font-weight: 600; text-align: center;">شماره رول: ${data.rollNumber}</div>
          </div>
        `;
      }
    ...
    markers: {
      size: 2, // Reduced for better performance
      hover: {
        size: 3
      },
      fillOpacity: 1,
      ...
      custom: function({ seriesIndex, dataPointIndex, w }: any) {
        ...
        return `
          <div class="custom-tooltip" style="text-align: right;background: white; padding: 12px; border: 1px solid #ccc; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            ...
            <div style="margin-bottom: 4px; color: #6B7280;">تاریخ: <span style="font-weight: 600; color: #111827;">${data.date || 'نامشخص'}</span></div>
            <div style="border-top: 1px solid #E5E7EB; padding-top: 8px; color: #374151; font-weight: 600; text-align: center;">شماره رول: ${data.rollNumber}</div>
            <div style="margin-bottom: 8px; color: #6B7280;">زمان نمونه‌گیری پایین: <span style="font-weight: 600; color: #111827;">${data.lowerSamplingTime || 'نامشخص'}</span></div>
          </div>
        `;
      }
    },
      ...
      custom: function({ seriesIndex, dataPointIndex, w }: any) {
        ...
        return `
          <div class="custom-tooltip" style="text-align: right;background: white; padding: 12px; border: 1px solid #ccc; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            ...
            <div style="margin-bottom: 4px; color: #6B7280;">تاریخ: <span style="font-weight: 600; color: #111827;">${data.date || 'نامشخص'}</span></div>
            <div style="border-top: 1px solid #E5E7EB; padding-top: 8px; color: #374151; font-weight: 600; text-align: center;">شماره رول: ${data.rollNumber}</div>
            <div style="margin-bottom: 8px; color: #6B7280;">زمان شروع نمونه‌گیری: <span style="font-weight: 600; color: #111827;">${data.samplingStartTime || 'نامشخص'}</span></div>
          </div>
        `;
      }
    },
