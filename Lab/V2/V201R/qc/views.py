...
class QCRecordViewSet(viewsets.ModelViewSet):
    ...
    def recent_papers(self, request):
        ...
        Supports searching by roll number.
        """
        ...
        # Get papers that haven't been used in QC records yet
        # Exclude current QC record if editing (to allow re-selection of same papers)
        exclude_qc_id = request.query_params.get('exclude_qc_id', None)
        used_paper_ids_query = QCRecord.objects.values_list('rollnumbers', flat=True)
        
        if exclude_qc_id:
            # Exclude papers from all QC records except the one being edited
            used_paper_ids_query = used_paper_ids_query.exclude(id=exclude_qc_id)
        
        used_paper_ids = used_paper_ids_query
        if search_query:
            available_papers = papers.exclude(id__in=used_paper_ids)
        else:
            available_papers = papers.filter(created_at__gte=since).order_by('-created_at').exclude(id__in=used_paper_ids)
