...
export const LogsList: React.FC = () => {
  ...
  const getModelIcon = (modelName: string) => {
    switch (modelName) {
      ...
      case 'Loading':
      case 'QCRecord':
        return <FileText className="w-4 h-4 text-info-600" />;
      case 'PaperType':
        return <FileText className="w-4 h-4 text-warning-600" />;
      default:
  ...
  const getModelName = (modelName: string) => {
    switch (modelName) {
      ...
      case 'Customer':
        return 'مشتری';
      case 'Loading':
        return 'مشخصات بارگیری';
      case 'QCRecord':
        return 'رکورد کنترل کیفی';
      case 'PaperType':
        return 'نوع کاغذ';
      default:
        return modelName;
  ...
  const formatFieldValue = (value: string | null | undefined): string => {
    ...
    // If the value looks like a formatted JSON (contains "ماده" or "مقدار:"), return as is
    if (typeof value === 'string' && (value.includes('ماده') || value.includes('مقدار:'))) {
      return value;
    }
    ...
    if (typeof value === 'string' && (value.startsWith('{') || value.startsWith('['))) {
      try {
        ...
        if (typeof parsed === 'object' && parsed !== null) {
          // Format material_usage style JSON
          if (Object.keys(parsed).every(key => /^\d+$/.test(key))) {
            const items: string[] = [];
            Object.entries(parsed).forEach(([key, data]: [string, any]) => {
              if (typeof data === 'object' && data !== null) {
                const parts: string[] = [];
                if (data.val !== undefined && data.val !== null) parts.push(`مقدار: ${data.val}`);
                if (data.brand) parts.push(`برند: ${data.brand}`);
                if (data.text) parts.push(`توضیحات: ${data.text}`);
                if (data.Soluble_in_water !== undefined && data.Soluble_in_water !== null) {
                  parts.push(`محلول در آب: ${data.Soluble_in_water}`);
                }
                if (parts.length > 0) {
                  items.push(`ماده ${key}: ${parts.join(', ')}`);
                }
              }
            });
            return items.length > 0 ? items.join(' | ') : value;
          }
        }
      } catch {
        ...
        <div className="card-body">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            >
              ...
              <option value="Loading">مشخصات بارگیری</option>
              <option value="QCRecord">رکورد کنترل کیفی</option>
              <option value="PaperType">نوع کاغذ</option>
            </select>
              ...
              {logs.map(log => (
                <div key={log.id} className="p-4 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors">
                  <div className="flex items-start gap-10">
                    {/* Right: User detail and action type */}
                    <div className="flex-shrink-0 text-right min-w-[150px]">
                          ...
                          {log.details.map((d, idx) => (
                            <div key={idx}>
                              {log.actionType === 'create' ? (
                                <div>
                                  {d.roll_number && <span>رول شماره {d.roll_number} | </span>}
                                  <span>فیلد "{d.name || '-'}" با مقدار 
                                    <span className="inline-block px-2 py-0.5 mx-1 bg-success-100 text-success-700 rounded font-medium">
                                      "{formatFieldValue(d.new || null)}"
                                    </span>
                                    ایجاد شد
                                  </span>
                                </div>
                                ...
                                <div>
                                  {d.roll_number && <span>رول شماره {d.roll_number} | </span>}
                                  <span>فیلد "{d.name || '-'}" از 
                                    <span className="inline-block px-2 py-0.5 mx-1 bg-red-100 text-red-700 rounded line-through font-medium">
                                      "{formatFieldValue(d.old || null)}"
                                    </span>
                                    به 
                                    <span className="inline-block px-2 py-0.5 mx-1 bg-success-100 text-success-700 rounded font-medium">
                                      "{formatFieldValue(d.new || null)}"
