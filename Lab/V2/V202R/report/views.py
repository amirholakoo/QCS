...
def complete_report_export_xlsx(request):
    ...
    # Define headers - Paper columns (24) + Pulp columns (18 + dynamic location columns)
    paper_headers = [
        'نوع رکورد',
        'شماره رول',
        'خط تولید',
    ...
    for paper in papers:
        ...
        ws.cell(row=row_num, column=col_num, value=paper_type_map.get(paper.paper_type, '') if paper.paper_type else ''); col_num += 1
        ...
        ws.cell(row=row_num, column=col_num, value=f"{paper.cylinder_temperature_before_press or '-'} - {paper.cylinder_temperature_after_press or '-'}"); col_num += 1
        ...
        for pulp in matching_pulps:
            ...
            for _ in range(21):  # Skip paper-specific columns
                col_num += 1
