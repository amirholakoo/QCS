...
class PaperViewSet(viewsets.ModelViewSet):
    ...
    def export_xlsx(self, request):
        ...
        for row_num, paper in enumerate(queryset, 2):
            ...
            ws.cell(row=row_num, column=20, value=', '.join(paper_temp_starch_values) if paper_temp_starch_values else '')
            ws.cell(row=row_num, column=21, value=', '.join(paper_temp_pop_reel_values) if paper_temp_pop_reel_values else '')
            ws.cell(row=row_num, column=22, value=paper.burst_test or '')
            ws.cell(row=row_num, column=23, value=paper.tensile_strength_md or '')
            ws.cell(row=row_num, column=24, value=paper.tensile_strength_cd or '')
            ws.cell(row=row_num, column=25, value=paper.cct1 or '')
            ws.cell(row=row_num, column=26, value=paper.cct2 or '')
            ws.cell(row=row_num, column=27, value=paper.cct3 or '')
            ws.cell(row=row_num, column=28, value=paper.cct4 or '')
            ws.cell(row=row_num, column=29, value=paper.cct5 or '')
            ws.cell(row=row_num, column=30, value=paper.rct1 or '')
            ws.cell(row=row_num, column=31, value=paper.rct2 or '')
            ws.cell(row=row_num, column=32, value=paper.rct3 or '')
            ws.cell(row=row_num, column=33, value=paper.rct4 or '')
            ws.cell(row=row_num, column=34, value=paper.rct5 or '')
            ws.cell(row=row_num, column=35, value=paper.tearing_time or '')
            ws.cell(row=row_num, column=36, value=paper.ProductionDowntime or '')
            ws.cell(row=row_num, column=37, value=paper.CauseOfTearing or '')
            ws.cell(row=row_num, column=38, value='بله' if paper.calender_applied else 'خیر')
            ws.cell(row=row_num, column=39, value=paper.machine_speed or '')
            ws.cell(row=row_num, column=40, value=format_material_usage(paper.material_usage))
            ws.cell(row=row_num, column=41, value=paper.user.username if paper.user else '')
            ws.cell(row=row_num, column=42, value=to_jalali_datetime(paper.created_at))
            ws.cell(row=row_num, column=43, value=to_jalali_datetime(paper.last_updated))
