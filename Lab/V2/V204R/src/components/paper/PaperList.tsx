    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm sm:text-base text-gray-600 mt-1">
            نمایش {formatPersianDate(filteredPapers.length.toString())} از {formatPersianDate(totalCount.toString())} رکورد
      ...
      <div className="card">
        <div className="card-body p-0">
                ...
                <tbody>
                  {filteredPapers.map((paper, index) => (
                    <tr 
                      ...
                      ref={index === filteredPapers.length - 1 && pageSize !== 'all' ? lastElementRef : null}
                    >
                      <td>
                            ...
                            {paper.pm_settings.map((setting, idx) => (
                              <div key={setting.id || idx} className="mb-1 pb-1 border-b border-gray-200 last:border-0">
                                ...
                                {(setting.cylinder_temperature_before_press || setting.cylinder_temperature_after_press) && (
                                  <div>دما سیلندر: {setting.cylinder_temperature_before_press || '-'}/{setting.cylinder_temperature_after_press || '-'}</div>
                                )}
                                {(setting.paper_temperature_before_starch || setting.paper_temperature_before_pop_reel) && (
                                  <div>دما کاغذ: {setting.paper_temperature_before_starch || '-'}/{setting.paper_temperature_before_pop_reel || '-'}</div>
                                )}
                              </div>
                            ))}
                          </div>
          ...
          {/* End of List Message */}
          {!loadingMore && !hasMore && filteredPapers.length > 0 && (
