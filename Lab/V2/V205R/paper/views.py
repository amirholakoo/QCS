...
class PaperViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if self.request.user.is_authenticated:
            try:
                log_action(self.request.user.username, 'Paper', 'create',[{'roll_number':paper.roll_number}])
            except:
                pass
    ...
    def export_xlsx(self, request):
        ...
        for row_num, paper in enumerate(queryset, 2):
            ...
            ws.cell(row=row_num, column=18, value=paper.density_valve or '')
            ws.cell(row=row_num, column=19, value=paper.diluting_valve or '')
            ...
            ws.cell(row=row_num, column=20, value=', '.join(temp_before_values) if temp_before_values else '')
            ws.cell(row=row_num, column=21, value=', '.join(temp_after_values) if temp_after_values else '')
            ws.cell(row=row_num, column=22, value=', '.join(paper_temp_starch_values) if paper_temp_starch_values else '')
            ws.cell(row=row_num, column=23, value=', '.join(paper_temp_pop_reel_values) if paper_temp_pop_reel_values else '')
            ws.cell(row=row_num, column=24, value=paper.burst_test or '')
            ws.cell(row=row_num, column=25, value=paper.tensile_strength_md or '')
            ws.cell(row=row_num, column=26, value=paper.tensile_strength_cd or '')
            ws.cell(row=row_num, column=27, value=paper.cct1 or '')
