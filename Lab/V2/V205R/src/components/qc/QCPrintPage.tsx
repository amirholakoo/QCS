...
export const QCPrintContent: React.FC<QCPrintContentProps> = ({
  ...
  const getInitialColumnOrder = (): string[] => {
    if (data.column_order && data.column_order.length > 0) {
      // Validate that all saved column order items exist in custom_fields_info
      ...
      // Add any missing fields from custom_fields_info that aren't in saved order
    ...
    return data.custom_fields_info.map((field) => field.field_name);
  };
  ...
  useEffect(() => {
    ...
    if (data.column_order && data.column_order.length > 0) {
      ...
      setColumnOrder(data.custom_fields_info.map((field) => field.field_name));
    }
