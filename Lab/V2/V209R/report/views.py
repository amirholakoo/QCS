...
def complete_report_export_xlsx(request):
    ...
    # Helper function to format material usage (includes Soluble_in_water when present)
    ...
    for paper in papers:
        ...
        ws.cell(row=row_num, column=col_num, value=paper.responsible_person_name or ''); col_num += 1
        ...
        if paper.profile_details:
            try:
                ...
                pass
        ws.cell(row=row_num, column=col_num, value=profile_details_str); col_num += 1
        ...
        ws.cell(row=row_num, column=col_num, value=paper.tearing_time or ''); col_num += 1
        ws.cell(row=row_num, column=col_num, value=paper.ProductionDowntime or ''); col_num += 1
        ws.cell(row=row_num, column=col_num, value=paper.CauseOfTearing or ''); col_num += 1
        ...
        # Paper temperature values (نشاسته/پوپ ریل)
        ...
        # دمای نشاسته/خشک کن ۳: fructose_temperature_before_press, paper_temperature_before_dryer3, dryer3_first_cylinder_temperature
        extra_temp_parts = []
        ...
        ws.cell(row=row_num, column=col_num, value=', '.join(extra_temp_parts) if extra_temp_parts else ''); col_num += 1
        # غلظت 1-5 (format like UI: 1: v1, 2: v2, ...)
        ...
        ws.cell(row=row_num, column=col_num, value=', '.join(density_parts) if density_parts else ''); col_num += 1
        ...
        ws.cell(row=row_num, column=col_num, value=', '.join(diluting_parts) if diluting_parts else ''); col_num += 1
...
PLC_ROLLS_API_URL = ["http://192.168.2.46:6010/api/rolls/"]
