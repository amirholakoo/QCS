...
class PaperViewSet(viewsets.ModelViewSet):
    ...
    def export_xlsx(self, request):
        ...
        for row_num, paper in enumerate(queryset, 2):
            ...
            ws.cell(row=row_num, column=11, value=getattr(roll_plc, 'paper_breaks', None) or '')
            ws.cell(row=row_num, column=12, value=getattr(roll_plc, 'printed_length', None) or '')

            ws.cell(row=row_num, column=13, value=paper.real_grammage or '')
            ws.cell(row=row_num, column=14, value=paper.humidity or '')
            ws.cell(row=row_num, column=15, value=paper.ash_percentage or '')
            ws.cell(row=row_num, column=16, value=paper.cub or '')
            ws.cell(row=row_num, column=17, value=profile_map.get(paper.profile, paper.profile or '') if paper.profile else '')
            
            # Format profile_details
            profile_details_str = ''
            if paper.profile_details:
                try:
                    ...
                    pass
            ws.cell(row=row_num, column=18, value=profile_details_str)
            
            ws.cell(row=row_num, column=19, value=paper.density_valve or '')
            ws.cell(row=row_num, column=20, value=paper.diluting_valve or '')
            ...
            ws.cell(row=row_num, column=21, value=', '.join(temp_before_values) if temp_before_values else '')
            ws.cell(row=row_num, column=22, value=', '.join(temp_after_values) if temp_after_values else '')
            ws.cell(row=row_num, column=23, value=', '.join(paper_temp_starch_values) if paper_temp_starch_values else '')
            ws.cell(row=row_num, column=24, value=', '.join(paper_temp_pop_reel_values) if paper_temp_pop_reel_values else '')
            ws.cell(row=row_num, column=25, value=paper.burst_test or '')
            ws.cell(row=row_num, column=26, value=paper.tensile_strength_md or '')
            ws.cell(row=row_num, column=27, value=paper.tensile_strength_cd or '')
            ws.cell(row=row_num, column=28, value=paper.cct1 or '')
            ws.cell(row=row_num, column=28, value=paper.cct2 or '')
            ws.cell(row=row_num, column=29, value=paper.cct3 or '')
            ws.cell(row=row_num, column=30, value=paper.cct4 or '')
            ws.cell(row=row_num, column=31, value=paper.cct5 or '')
            ws.cell(row=row_num, column=32, value=paper.rct1 or '')
            ws.cell(row=row_num, column=33, value=paper.rct2 or '')
            ws.cell(row=row_num, column=34, value=paper.rct3 or '')
            ws.cell(row=row_num, column=35, value=paper.rct4 or '')
            ws.cell(row=row_num, column=36, value=paper.rct5 or '')
            ws.cell(row=row_num, column=37, value=paper.tearing_time or '')
            ws.cell(row=row_num, column=38, value=paper.ProductionDowntime or '')
            ws.cell(row=row_num, column=39, value=paper.CauseOfTearing or '')
            ws.cell(row=row_num, column=40, value='بله' if paper.calender_applied else 'خیر')
            ws.cell(row=row_num, column=41, value=paper.machine_speed or '')
            ws.cell(row=row_num, column=42, value=format_material_usage(paper.material_usage))
            ws.cell(row=row_num, column=43, value=paper.user.username if paper.user else '')
            ws.cell(row=row_num, column=44, value=to_jalali_datetime(paper.created_at))
            ws.cell(row=row_num, column=45, value=to_jalali_datetime(paper.last_updated))
...
class ProductionMachineViewSet(viewsets.ModelViewSet):
    ...
    def perform_destroy(self, instance):
        """
        Delete production machine and log the action.
        """
