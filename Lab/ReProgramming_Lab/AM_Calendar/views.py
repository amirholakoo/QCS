...
def calendar(form=False,request=False,year=False,month=False,Json_response=False):
    ...
    if not month or month== '0':
        ...
        month = int(month) - 1
    # if request.user.groups.filter(name = 'presenter').exists():
    #     all_reserv = all_reserv.filter(presenter__user=request.user)
    calendar = []
    ...
    if y+1 < 7:
        ...
        for z in range(31):
            ...
            # for r in all_reserv:
            #     if str(YEARS[x]) == str(r.year) and MONTH_NAME[y].encode('utf-8') == r.get_month_display().encode('utf-8') and z+1 == int(r.day):
            #         i['month'][0]['days'][z]['reserved'].append({'presenter':f'{r.presenter.user.get_full_name()}','beganhours':f'{r.beganhours}'})
    else:
        for z in range(30):
            i['month'][0]['days'].append({'title':f'{get_fa(int(YEARS[x]),y+1,z+1)}','active':False,'reserved':[]})
            if YEARS[x] == NOW[0] and y+1 == int(NOW[1]) and z+1 == int(NOW[2]):
                ...
                curent = {'year':[{'name':str(int(YEARS[x]) -1),'active':False},{'name':YEARS[x],'active':True},{'name':str(int(YEARS[x]) +1),'active':False}],'month':y+1}
            # for r in all_reserv:
            #     if YEARS[x] == r.year and MONTH_NAME[y].encode('utf-8') == r.get_month_display().encode('utf-8') and z+1 == int(r.day):
            #         i['month'][0]['days'][z]['reserved'].append({'presenter':f'{r.presenter.user.get_full_name()}','beganhours':f'{r.beganhours}'})
