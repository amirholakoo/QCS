...
def save_thermal_image(data):
    ...
    if data is None:
        ...
        DATA_IS_CORRECT = True
    # Process MLX90640 data (24x32 dimensions)
    # Handle different API response formats
    if "data" in data and "heatmap" in data["data"]:
        temp_data = data["data"]["heatmap"]
    elif "temperature" in data:
        temp_data = data["temperature"]
    else:
        return JsonResponse({"error": "Unexpected data format"}, status=500)
    
    arr = np.array(temp_data).reshape((24, 32))
    arr = np.fliplr(arr)
    ...
    img = np.uint8(norm)
    # Resize to maintain aspect ratio for MLX90640 (24x32 -> 480x640)
