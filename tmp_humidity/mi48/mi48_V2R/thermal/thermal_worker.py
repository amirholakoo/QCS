...
def save_thermal_image(data):
    ...
    if data is None:
        ...
        DATA_IS_CORRECT = True
    # frame = np.array(data).reshape((settings.THERMAL_SENSOR_ROWS, settings.THERMAL_SENSOR_COLS))
    ...
    arr = np.array(data["temperature"]).reshape((settings.THERMAL_SENSOR_ROWS, settings.THERMAL_SENSOR_COLS))
    arr = np.fliplr(arr)
