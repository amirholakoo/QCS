...
def thermal_view(request):
    ...
    chart_data = []
    label_data = []
    roll_annotations = []  # Store roll number changes
    current_roll = None
    avg = 0
    index = 0
    time = None
    ...
    print(roll_last_time.id)
    roll_Remainder = roll_last_time.timestamp.timestamp() + config.roll_Duration - timezone.now().timestamp() if roll_last_time else config.roll_Duration
    print(roll_Remainder,roll_last_time.timestamp.timestamp() + config.roll_Duration - timezone.now().timestamp())
    if roll_Remainder < 1 :
        roll_Remainder = 1
    ...
    return render(request,"thermal/index.html",context={"log":logs,"formuls":FORMULA,"chart_data":chart_data,"label_data":label_data,"max": len(chart_data),"threshold":threshold,"edge_threshold":edge_threshold,"roll_number":roll_number,"roll_Remainder":int(roll_Remainder),"roll_Duration":config.roll_Duration,"roll_annotations":roll_annotations})
...
def save_probe_data(request):
    ...
    now = time.time()
    if now - ProbeData.objects.last().timestamp.timestamp() < 60:
...
def auto_save_probe_data():
    ...
    try:
        # Get current probe configuration
        config = ProbeConfiguration.objects.first()
        last_probe_data = ProbeData.objects.last()
        now = time.time()
        print(last_probe_data.timestamp.timestamp(),"________________auto_save_probe_data")
        if now - last_probe_data.timestamp.timestamp() < 60:
            return
...
def save_roll_number(request):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            roll_number = data.get('roll_number')
            ProbeData.objects.last().roll_number = roll_number
            print(roll_number,"________________save_roll_number")
            ProbeData.objects.last().save()
            config = ProbeConfiguration.objects.first()
            config.roll_number = roll_number
            config.save()
            return JsonResponse({'status': 'success', 'roll_number': roll_number})
        except Exception as e:
            print(e,"________________save_roll_number error")
            return JsonResponse({'error': str(e)}, status=500)
    else:
        return JsonResponse({'error': 'Invalid method'}, status=405)